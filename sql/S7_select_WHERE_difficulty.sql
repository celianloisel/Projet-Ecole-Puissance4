...
-- DESC : Change le tri par default et fait passer en ordre descendant
SELECT pseudo, game, difficulty, score FROM scores INNER JOIN users ON scores.user_id = users.id INNER JOIN games ON scores.game_id = games.id WHERE difficulty = "" ORDER BY game, difficulty, score DESC